...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	/** Names of beans that have already been created at least once 已经创建的beans*/
	...
	/**
	 * Return an instance, which may be shared or independent(共享或独立), of the specified bean.
	 * @param name the name of the bean to retrieve
	 * @param requiredType the required type of the bean to retrieve
	 * @param args arguments to use if creating a prototype using explicit arguments to a
	 * static factory method. It is invalid to use a non-null args value in any other case.
	...
	protected <T> T doGetBean(
		...
		if (sharedInstance != null && args == null) {
			...
			//如果在beanDefinitionMap中不包括beanName,则尝试从parentBeanFactory中检测
	 ...
	 * and resolving aliases to canonical names(将别名解析成规范名称).
	...
	protected Object getObjectForBeanInstance(
		...
		//如果指定的name是工厂相关（以&为前缀），且beanInstance又不是FatoryBean类型则验证不通过
		...
		//现在我们有了bean的实例，这个实例可能是正常的bean或者factoryBean
		//如果是正常的bean，则返回该bean
		//如果是FactoryBean，我们用它创建实例，除非调用者真正是想要一个factory实例
		if (!(beanInstance instanceof FactoryBean) || BeanFactoryUtils.isFactoryDereference(name)) {
			return beanInstance;
		}
		...
		if (object == null) {
			...
			if (mbd == null && containsBeanDefinition(beanName)) {
				//将存储xml配置文件的GernericaBeanDefinition转换为RootBeanDefinition，如果指定BeanName是子bean的话同时会合并父类的相关属性
