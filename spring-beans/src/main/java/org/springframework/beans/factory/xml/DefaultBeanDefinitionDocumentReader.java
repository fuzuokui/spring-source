...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	protected BeanDefinitionParserDelegate createDelegate(
		...
		//delegate  百度翻译为：代表，代表团成员; fuzuokui
	...
	protected void parseBeanDefinitions(Element root, BeanDefinitionParserDelegate delegate) {
		//处理spring默认的命名空间。即spring定义的标签 fuzuokui
	...
	protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
		//由BeanDefinitionParserDelegate去解析bean
		BeanDefinitionHolder bdHolder = delegate.parseBeanDefinitionElement(ele);
		if (bdHolder != null) {
			//如果需要的话，对beanDefinition进行装饰。对自定义标签进行处理
			bdHolder = delegate.decorateBeanDefinitionIfRequired(ele, bdHolder);
			try {
				// Register the final decorated instance.
				//注册beanDefinition。BeanDefinitionRegistry是哪个具体类的对象？
			...
			//通知监听器解析及注册完成。用于扩展，spring没做逻辑处理
	 ...
	 */
	/**
	 * 空方法。子类可以实现该方法，将自定义标签转换成spring的标准标签，供后续解析。fuzuokui
	 * @param root
     */
